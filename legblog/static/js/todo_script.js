$(document).ready(function () {
    ...
    if (window.location.hash === '') {
        window.location.hash = '#todoism'; // home page, show the default view
    } else {
    ...
    function login_user() {
        ...
        $.ajax({
            ...
            contentType: 'application/json;charset=UTF-8',
            success: function (data) {
                if (window.location.hash === '#todoism' || window.location.hash === 'todoism') {
                    $(window).trigger('hashchange');
                } else {
                    window.location.hash = '#todoism';
                }
                activeM();
                M.toast({html: data.message});
            }
