...
def manage_post():
    ...
    pagination = Post.query.order_by(Post.timestamp.desc()).paginate(
        page, per_page=current_app.config['LEGBLOG_MANAGE_POST_PER_PAGE'])
...
@admin_bp.route('/comment/<int:comment_id>/hide', methods=['POST'])
@login_required
def approve_hide(comment_id):
    comment = Comment.query.get_or_404(comment_id)
    comment.reviewed = False
    db.session.commit()
    flash('该评论已被隐藏', 'warning')
    return redirect_back()
